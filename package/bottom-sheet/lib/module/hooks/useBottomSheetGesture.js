...
import { resolveGestureEnd } from '../utils/gestureDecision';
...
export const useBottomSheetGesture = ({
  ...
  const createPanGesture = useCallback(enabled => {
    return Gesture.Pan().enabled(enabled).onBegin(() => {
  ...
  const handlePanGesture = useMemo(() => createPanGesture(renderContent), [renderContent, createPanGesture]);
  const contentPanGesture = useMemo(() => createPanGesture(renderContent && enableContentGesture), [renderContent, enableContentGesture, createPanGesture]);
  return {
    handlePanGesture,
    contentPanGesture
  };
