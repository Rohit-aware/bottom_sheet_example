 ...
 */
export const useKeyboardBehavior = visible => {
  const keyboardOffset = useSharedValue(0);
  const animateKeyboardOffset = useStableCallback(offset => {
    keyboardOffset.value = withTiming(offset, {
      duration: 250,
      easing: Easing.inOut(Easing.ease)
    });
  });
  const keyboardDidShow = useStableCallback(event => {
  ...
  useEffect(() => {
    ...
    listeners.push(Keyboard.addListener(showEvent, keyboardDidShow), Keyboard.addListener(hideEvent, keyboardDidHide));
    return () => {
