...
exports.resolveGestureEnd = void 0;
 ...
 */
const resolveGestureEnd = (input, thresholds) => {
  'worklet';

  const activeThresholds = thresholds ?? _defaults.DEFAULT_GESTURE_THRESHOLDS;
  const {
    velocityY,
    currentTranslateY,
  ...
  // Close decision
  const shouldCloseFromLower = velocityY > activeThresholds.closeVelocity || draggedDown > maxHeight * activeThresholds.lowerSnapCloseRatio;
  const shouldCloseFromTop = velocityY > activeThresholds.closeFromTopVelocity || projectedY > snap0TranslateY + (maxHeight - snap0TranslateY) * activeThresholds.topSnapCloseRatio;
  const shouldClose = !hasMultipleSnaps ? shouldCloseFromLower : isAtTopSnap ? shouldCloseFromTop : shouldCloseFromLower;
  if (shouldClose) {
    return {
      action: 'close'
    };
...
exports.resolveGestureEnd = resolveGestureEnd;
//# sourceMappingURL=gestureDecision.js.map